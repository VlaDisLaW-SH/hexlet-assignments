...
public class ProductSpecification {
    public Specification<Product> bild(ProductParamsDTO params) {
        return withCategoryId(params.getCategoryId())
            .and(withPriceGt(params.getPriceGt()))
            .and(withPriceLt(params.getPriceLt()))
            .and(withRatingGt(params.getRatingGt()))
            .and(withTitleContSubStr(params.getTitleCont()));
    }
    ...
    private Specification<Product> withTitleContSubStr(String subStr) {
        return (root, query, cb) -> subStr == null ? cb.conjunction() : cb.like(root.get("title"), subStr);
    }
}
// END
